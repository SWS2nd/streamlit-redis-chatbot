...
with st.sidebar:
    session_id = st.text_input("session ID",value="example0724")
...
if user_input:= st.chat_input("텍스트를 입력하세요."):
    ...
    with st.chat_message("assistant"):
        ...
        # 1. LLM 모델 생성
        # chatopenai를 chatupstage로, gpt를 solar-mini로 변경, api키 가져오기
        api_key = os.getenv("UPSTAGE_API_KEY")
        llm = ChatUpstage(streaming=True, callbacks=[stream_handler],model='solar-mini',api_key=api_key)
